...
class TestPortfolioOptimization:
    ...
    def test_calculate_mean_variance_single_asset(self) -> None:
        """Test calculating mean and covariance for single asset."""
        dates = pd.date_range("2024-01-01", periods=100, freq="D")
        df = pd.DataFrame(
            {
                "Price": np.random.randn(100) * 10 + 100,
    ...
    def test_optimize_portfolio_mean_variance_basic(self) -> None:
        ...
        assert all(w >= 0 and w <= 1 for w in optimal_weights)
        assert "ASSET1" in optimal_weights.index
    ...
    def test_optimize_portfolio_mean_variance_with_minimum_allocation(self) -> None:
        ...
        assert len(optimal_weights) == 2
        assert np.isclose(optimal_weights.sum(), 1.0, rtol=1e-5)
        assert all(w >= min_allocation for w in optimal_weights)
    ...
    def test_optimize_portfolio_mean_variance_single_asset(self) -> None:
        ...
        assert np.isclose(optimal_weights.iloc[0], 1.0, rtol=1e-5)
        assert "ASSET1" in optimal_weights.index
